...
double CgdaExecutionIET::getCustomFitness(vector <double> genPoints){
...
void CgdaExecutionIET::registerParameters(StateP state) {
	state->getRegistry()->registerEntry("function", (voidP) (new uint(1)), ECF::UINT);
}

/************************************************************************/

bool CgdaExecutionIET::initialize(StateP state) {
...
FitnessP CgdaExecutionIET::evaluate(IndividualP individual) {
